...
class BillingModel extends CI_Model 
{
	...
	function getPatientCptRate($post_data = array())
	{
		...
		if ($post_data["cpt_code_ids"])
		{
			...
			//$this->db->group_by("C.CURRENT_PROCEDURAL_CODE_ID");
			...
			if($query->num_rows() > 0)
			{
				...
				// foreach ($cpt_code_ids as $key => $cpt) {
				// 	foreach ($re_array as $keys => $value) {
				// 		if($cpt != $value->rate)
				// 		{
				// 			$re_array[$key+1] = 0;
	...
	function generatePatientBill($post_data = array())
	{
		...
		if(!empty($post_data))
		{
			...
			"BILLING_DATE" 		=> toUtc(trim($post_data["date"]),$post_data["timeZone"]), //date("Y-m-d H:i:s"),//trim($post_data["billing_date"]),
			...
			else
			{
				...
				if($post_data["patient_type"] == 1 && count($post_data["cpt_code_ids"])>0)
				{
	...
	function savePatientBill($post_data = array())
	{
		...
		if(!empty($post_data))
		{
			if(isset($post_data["claim_invoice"]) && count($post_data["claim_invoice"]) > 0)
			{
				...
				"BILLING_DATE" 		=> toUtc(trim($claim_invoice["date"]),$claim_invoice["timeZone"]), //date("Y-m-d H:i:s"),//trim($claim_invoice["billing_date"]),
				//"PENDING_PAYMENT" 	=> trim($claim_invoice["balance_payment"]),
				...
				if ($data_id> 0)
				{
					//$this->updatependingpayment($claim_invoice["patient_id"]);
					$this->db->where("BILLING_ID",$data_id);
					$this->db->update("BILLING",$data);
			...
			if(isset($post_data["cash_invoice"]) && count($post_data["cash_invoice"]) > 0)
			{
				...
				"BILLING_DATE" 		=> toUtc(trim($cash_invoice["date"]),$cash_invoice["timeZone"]), //date("Y-m-d H:i:s"),//trim($cash_invoice["billing_date"]),
	...
	{	
		...
		//$lab_details_id = $post_data["lab_details_id"];
		$billing_details_ids = $post_data["billing_details_ids"];
		$prior_authorization = $post_data["prior_authorization"];
		//$test_result =  $post_data["test_result"];
			...
			if(is_array($save_details) && count($save_details) > 0)
			{
				foreach ($save_details as $key => $value) {
					...
					if($value["EDIT"] == true)
					{
						...
						{
							$ins_array = array(
								//"REQ_ID"=>$prior_authorization[$key],
								//"LAB_INVESTIGATION_DETAILS_ID"=>$prior_authorization[$key],
			...
			if (isset($post_data["assessment_id"]) &&  $post_data["assessment_id"] > 0)
			{
				...
				if($query->num_rows() > 0)
				{
					...
					foreach ($bill_data as $key => $value) {
						# code...
						$bill_data[$key]["bill_details"] = $this->getBillDetails($value["BILLING_ID"]);
	...
	public function getPatient($bill_data)
	{
		...
		/*if (!empty($bill_data))
 		{
 			// $bill_data["PATIENT_TYPE_DETAIL_ID"]
 			if($bill_data["PATIENT_TYPE"] == 1)
 			{
 				$this->db->start_cache();
				...
				$this->db->select("IN.*,INP.INSURANCE_PAYERS_NAME,INN.INS_NETWORK_NAME,RT.OPTIONS_NAME as PAY_TYPE");
				$this->db->from("OP_INS_DETAILS IN");
				$this->db->join("OPTIONS RT","RT.OPTIONS_ID = '".$bill_data["PATIENT_TYPE"]."'","left");
				$this->db->join("INSURANCE_PAYERS INP","INP.INSURANCE_PAYERS_ID = IN.OP_INS_PAYER","left");
				$this->db->join("INS_NETWORK INN","INN.INS_NETWORK_ID = IN.OP_INS_NETWORK","left");
 			...
 			else if($bill_data["PATIENT_TYPE"] == 3)
 			{
 				$this->db->start_cache();
	 			$this->db->where("IN.OP_REGISTRATION_ID",$bill_data["PATIENT_ID"]);	
	 			$this->db->where("IN.OP_CORPORATE_DETAILS_ID",$bill_data["PATIENT_TYPE_DETAIL_ID"]);	
				$this->db->select("IN.*,INP.INSURANCE_PAYERS_NAME,INN.INS_NETWORK_NAME,RT.OPTIONS_NAME as PAY_TYPE");
				$this->db->from("OP_CORPORATE_DETAILS CD");
				$this->db->join("CORPORATE_COMPANY CC","CC.CORPORATE_COMPANY_ID = CD.OP_CORPORATE_COMPANY_ID",'left');
				$this->db->join("OPTIONS RT","RT.OPTIONS_ID = '".$bill_data["PATIENT_TYPE"]."'","left");
 			...
 			{		
 				$this->db->start_cache();
				$this->db->where("OP.OP_REGISTRATION_ID",$bill_data["PATIENT_ID"]);	
				$this->db->select("RT.OPTIONS_NAME as PAY_TYPE,OP.OP_REGISTRATION_TYPE");
				$this->db->from("OP_REGISTRATION OP");
				$this->db->join("OPTIONS RT","RT.OPTIONS_ID = '".$bill_data["PATIENT_TYPE"]."'","left");
				$data = $this->db->get();
				$this->db->stop_cache();
	...
	public function assessmentListByDate($post_data = array())
	{
		...
		if(!empty($post_data))
		{
			...
			{
				//(SELECT B.BILL_STATUS FROM BILLING B JOIN NURSING_ASSESSMENT NA ON NA.NURSING_ASSESSMENT_ID = B.ASSESSMENT_ID LIMIT 1) AS BILL_STATUS,
				$this->db->start_cache();
				...
				$this->db->select("B.*,NA.*,OP.*,
					...
					(SELECT BILL_STATUS FROM BILLING WHERE BILLING.ASSESSMENT_ID = L.ASSESSMENT_ID LIMIT 1) AS BILL_STATUS");
				...
				$query = $this->db->get();
				// $this->db->group_by("B.PATIENT_ID,B.ASSESSMENT_ID");
				...
				if($query->num_rows() > 0)
				{
					...
					/*foreach ($data as $key => $value) {
						if($value["OP_REGISTRATION_TYPE"] == 1 &&  $value["PATIENT_TYPE"] == null || $value["PATIENT_TYPE"] != null &&  $value["PATIENT_TYPE"] == 1)
						{
							$data[$key]["insurance"] = $this->InsDetails($value["PATIENT_ID"],$value["INS_DETAIL"]);
						}
						else
						{
							$data[$key]["insurance"] = false;
	...
	public function invoiceList($post_data = array())
	{
		$result = array("status"=> "Failed", "data"=> array());		 
			$this->db->start_cache();
			...
			{
				$this->db->group_start();
					$this->db->like("B.INVOICE_NO",$post_data["search_text"]);
			...
			{
				$this->db->where("B.BILLING_DATE <= ",format_date($post_data["invoice_date_to"]));	
			}
			
			if ($post_data["invoice_date_from"] != "")
			{
				$this->db->where("B.BILLING_DATE >= ",format_date($post_data["invoice_date_from"]));	
			}
		...
		$ret = $this->ApiModel->mandatory_check( $post_data , array('patient_id','assessment_id','current_procedural_code_id'));		  
		   ...
		   {		  	 		  		                         	 		  
			...
			if($data_id>0)
			{
			 ...
			 // 	);	    									
				// if($this->utility->is_Duplicate("LAB_INVESTIGATION_DETAILS",array_keys($dc_data), array_values($dc_data),"LAB_INVESTIGATION_DETAILS_ID",$lab_investigation_details_id))
				// {								
				// 	return array("status"=> "Failed", "data_id"=>$data_id, "msg"=> "Duplicate Data Found");
				// }	
				...
				else
				{
					...
					// if($this->utility->is_Duplicate("LAB_INVESTIGATION_DETAILS",array_keys($dc_data), array_values($dc_data),"LAB_INVESTIGATION_ID",$data_id))
					// {								
					// 	return array("status"=> "Failed", "data_id"=>$data_id, "msg"=> "Duplicate Data Found");
					// }
					$this->db->start_cache();			
