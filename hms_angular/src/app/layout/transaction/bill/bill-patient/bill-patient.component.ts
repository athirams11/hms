...
export class BillPatientComponent implements OnInit {
  ...
  public getTotal() {
    ...
    for (let invoice of this.invoice_list) {
      ...
      if(invoice.DEPARTMENT_ID == 0 && invoice.GENERATED == 0 || invoice.DEPARTMENT_ID == null && invoice.GENERATED == null){
      ...
      else if(this.user_data.department != 0){
        bill_total = bill_total + total;
        gross_total = gross_total + item_total;
        patient_payable_total = patient_payable_total + patient_pay_tot;
        net_total = gross_total - patient_payable_total;
      }
       else if(invoice.GENERATED == 1 && this.user_data.department == 0){
        bill_total = bill_total + total;
        gross_total = gross_total + item_total;
        patient_payable_total = patient_payable_total + patient_pay_tot;
        net_total = gross_total - patient_payable_total;
      ...
      table_invoice_list.push({
        ...
        BILLING_INVOICE_NUMBER: invoice.BILLING_INVOICE_NUMBER,
  ...
  public correctprint(cash_full,printcashBill){
    this.cash_list=cash_full.bill_details;
    this.cash_gross_total = parseFloat(cash_full.BILLED_AMOUNT);
    this.cash_net_total = parseFloat(cash_full.INSURED_AMOUNT);
    this.cash_bill_total = parseFloat(cash_full.INSURED_AMOUNT);
    this.cash_patient_payable_total = parseFloat(cash_full.PATIENT_PAYABLE);
    this.payment_mode = cash_full.PAYMENT_MODE;
    this.cash_payment_mode = cash_full.PAYMENT_MODE;
    this.patient_pay_payment = cash_full.PAID_BY_PATIENT;
    this.pending_amount = parseFloat(cash_full.PENDING_PAYMENT);
    this.cash_invoice_number = cash_full.BILLING_INVOICE_NUMBER;
    this.open(printcashBill);
    
  }
  public cashprint(cash_full){
    this.cash_list=cash_full.bill_details;
    this.cash_gross_total = parseFloat(cash_full.BILLED_AMOUNT);
    this.cash_net_total = parseFloat(cash_full.INSURED_AMOUNT);
    this.cash_bill_total = parseFloat(cash_full.INSURED_AMOUNT);
  ...
  public correctclaimprint(credit_full,printclaimBill){
    this.claim_list=credit_full.bill_details;
    this.claim_gross_total = parseFloat(credit_full.BILLED_AMOUNT);
    this.claim_net_total = parseFloat(credit_full.INSURED_AMOUNT);
    this.claim_bill_total = parseFloat(credit_full.INSURED_AMOUNT);
    this.claim_patient_payable_total = parseFloat(credit_full.PAID_BY_PATIENT);
    this.payment_mode = credit_full.PAYMENT_MODE;
    this.claim_payment_mode = credit_full.PAYMENT_MODE;
    this.claim_invoice_number = credit_full.BILLING_INVOICE_NUMBER;
    this.open(printclaimBill);
    
  }

  public claimprint(credit_full){
    this.claim_list=credit_full.bill_details;
    this.claim_gross_total = parseFloat(credit_full.BILLED_AMOUNT);
    this.claim_net_total = parseFloat(credit_full.INSURED_AMOUNT);
    this.claim_bill_total = parseFloat(credit_full.INSURED_AMOUNT);
        ...
        for (let bill of result.data) {
          ...
          if (bill.BILL_TYPE == 1) {
            ...
            if(this.user_data.department==0){
              ...
              //console.log(this.credit_full_list);
            }
          ...
          if (bill.BILL_TYPE == 0) {
            ...
            if(this.user_data.department==0){
              ...
              //console.log(this.cash_full_list);
            }
  ...
  public savePatientBill() {
    if (this.cash_list.length > 0 && this.patient_pay_payment == 0 || this.patient_pay_payment < 0 || this.patient_pay_payment == null) {
      ...
      else if(this.user_data.department > 0 && this.cash_list.length > 0 && this.cash_patient_payable_total > this.patient_pay_payment){
        this.notifier.notify("error", "Paid amount is less than patient payable ")
      }
      else {
        if (this.cash_list.length == 0) {
          this.patient_pay_payment = 0
    ...
    if (this.cash_list.length > 0 && this.patient_pay_payment == 0 || this.patient_pay_payment < 0 || this.patient_pay_payment == null) {
      ...
      // else {
        ...
        if (this.patient_details.patient_data.OP_REGISTRATION_TYPE == 1) {
          ...
          //  console.log(this.cash_billing_id);
          ...
          if (this.cash_billing_id == 0 && this.claim_billing_id == 0 && this.user_data.department == 0) {
            this.generateclaimPatientBill();
            this.generatecashPatientBill();
            this.getBillByAssessment();
          }
  ...
  cptsearch = (text$: Observable<string>) =>
    text$.pipe(
      ...
      switchMap(term =>
        this.bill.cptsearch(term, this.user_data.department).pipe(
